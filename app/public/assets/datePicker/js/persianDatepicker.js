...
(function ($) {
    $.fn.persianDatepicker = function (options) {
        ...
        if (!instance) {
            return this.each(function () {
                return $(this).data(
                    pluginName,
                    new persianDatepicker(this, options)
                );
            });
        }
    ...
    var persianDatepicker = (function () {
        function persianDatepicker(element, userOptions) {
            ...
            if (
                self.options.selectableMonths._indexOf(
                    self._persianDate.month
                ) == -1
            )
                ...
                if (self.options.selectedDate != undefined) {
                    ...
                    self.showDate(
                        ...
                        self
                            ...
                            .toString(
                                "YYYY-MM-DD-" +
                                    self.jDateFunctions.getWeekday(self.now())
                            ),
                        self.now().gDate,
                        options.showGregorianDate
                    );
        ...
        persianDatepicker.prototype = {
            ...
            header: function () {
                ...
                _head = $(
                    '<div class="pdp-header" ' + self.headerStyle + " />"
                );
                ...
                if (
                    ...
                    _next.bind("click", function () {
                        ...
                        for (
                            ;
                            self.options.selectableMonths._indexOf(nextMonth) ==
                                -1 && nextMonth < 13;
                            nextMonth++
                        );
                        self.persianDate.addMonth(
                            nextMonth - self.persianDate.month
                        );
                        self.render();
                    });
                ...
                var _yearText = $("<span/>")
                    .html(
                        self.options.persianNumbers
                            ? self.jDateFunctions.toPersianNums(
                                  self.persianDate.year
                              )
                            : self.persianDate.year
                    )
                    ...
                    $.each(
                        ...
                        function (i, v) {
                            ...
                            o.bind("click", function () {
                                ...
                                if (
                                    _startDate.year == v ||
                                    _startDate.year == 9999
                                )
                                    self.persianDate.month = _startDate.month;
                                self.render();
                            });
                            pre
                                ? _yearSelect.prepend(o)
                                : _yearSelect.append(o);
                        }
                    );
                };
                getSelectableYears();
                ...
                for (i = 1; i <= 12; i++) {
                    ...
                    var o =
                        ...
                        (_startDate.year == self.persianDate.year &&
                            _startDate.month > i) ||
                        (_endDate.year == self.persianDate.year &&
                            i > _endDate.month)
                            ? $(
                            ...
                            : $("<li " + self.selectMonthLiStyle + " />").html(
                                  m
                              );
                    ...
                    for (var col = 0; col < 7; col++, cellIndex++) {
                        if (
                            ...
                            if (
                                self.options.startDate != null &&
                                (self.persianDate
                                    .parse(self.options.startDate)
                                    .compare(_dt) == -1 ||
                                    self.persianDate
                                        .parse(self.options.endDate)
                                        .compare(_dt) == 1)
                            )
                                _disday = "disday";
            ...
            footer: function () {
                var self = this;
                _footer = $(
                    '<div class="pdp-footer" ' + self.footerStyle + " />"
                );
                ...
                if (
                    ...
                    _goToday.attr(
                        "data-jdate",
                        self.now().toString("YYYY-MM-DD-DW")
                    );
                    _goToday.attr(
                        "data-gdate",
                        self.jDateFunctions.getGDate(self.now())
                    );
            ...
            showDate: function (el, jDate, gDate, showGdate) {
                var self = this;
                jDate = self.persianDate
                    .parse(jDate)
                    .toString(self.options.formatDate);
    ...
    persianDate.prototype = {
        ...
        addDay: function (d) {
            ...
            for (var i = 0; i < to; i++) {
                ...
                var lastDayOfMonth =
                    d > 0
                        ? jdf.getLastDayOfMonth(this)
                        : jdf.getLastDayOfMonth(r);
